...
public interface TaskEventRepository extends JpaRepository<TaskEvent, Long> {
    ...
    // For tasks
    List<TaskEvent> findByUserIdAndTypeAndDueDateBetween(Long userId, TaskEvent.TaskEventType type, Date start,
            Date end);

    // For events
    List<TaskEvent> findByUserIdAndTypeAndStartTimeBetween(Long userId, TaskEvent.TaskEventType type, Date start,
            Date end);
    ...
    // Find all items (for admin) by user
    @Query("SELECT te FROM TaskEvent te JOIN te.assignedUsers u WHERE u.id = :userId AND te.type = :type")
    List<TaskEvent> findByAssignedUserIdAndType(Long userId, TaskEvent.TaskEventType type);
