  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      {conversations.map((conversation) => (
        ...
        >
          <div className={`bg-${conversation.id === 1 ? 'blue' : conversation.id === 2 ? 'green' : conversation.id === 3 ? 'purple' : conversation.id === 4 ? 'red' : 'yellow'}-500 rounded-full h-12 w-12 flex items-center justify-center text-white font-bold`}>
            {conversation.avatar}
          </div>
          <div className="ml-3 flex-1">
            <div className="flex justify-between">
              <h3 className={`font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>{conversation.name}</h3>
            ...
            <div className="flex justify-between items-center">
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} truncate max-w-xs`}>
                {conversation.archived && <Archive size={12} className="inline mr-1" />}
